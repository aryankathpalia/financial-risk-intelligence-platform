<script lang="ts">
  ...
  const coreCapabilities = [
    {
      ...
      description:
        "Tuned for high recall so critical fraud signals are caught early with manageable analyst load.",
      icon: TriangleAlert,
      className:
        "border-sky-500/35 bg-gradient-to-br from-[#0b1220]/90 to-[#1a2240]/70 shadow-[0_10px_28px_rgba(56,189,248,0.14)]"
    },
    {
      title: "Validation-backed model quality",
      description:
        "Consistent ROC-AUC around 0.98 with false positives controlled near 1.2%.",
      icon: CircleGauge,
    ...
    },
    {
      ...
      description:
        "Alerts, evidence, and analyst decisions stay linked in one clear investigation workflow.",
      icon: BadgeAlert,
    ...
    },
    {
      ...
      description:
        "Ingestion, scoring APIs, and dashboard monitoring are built as a production workflow.",
      icon: Cpu,
      className:
        "border-slate-700/70 bg-gradient-to-br from-slate-900/85 via-slate-900/80 to-sky-950/20"
    }
  ] as const;
  ...
  const avgTransactionsPerMin = Math.round(
    activitySeries.reduce((acc, value) => acc + value, 0) / activitySeries.length
  );
  const peakTransactionsPerMin = Math.max(...activitySeries);
  const volatilityIndicator = Math.round(
    ((Math.max(...activitySeries) - Math.min(...activitySeries)) / avgTransactionsPerMin) * 100
  );
  ...
  <div class="min-h-[calc(100vh-3rem)] max-w-6xl mx-auto space-y-12">
    ...
    <section data-reveal class="reveal space-y-6 border-t border-white/5 pt-9">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5 items-stretch">
        {#each coreCapabilities as capability}
          <article class={`h-full rounded-2xl border p-6 backdrop-blur-sm transition duration-300 hover:scale-[1.02] hover:border-sky-500/35 hover:shadow-[0_10px_26px_rgba(56,189,248,0.14)] ${capability.className}`}>
            <div class="flex h-full flex-col">
              <span class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-700/70 bg-slate-900/70 text-sky-300">
                <svelte:component this={capability.icon} size={18} strokeWidth={1.9} aria-hidden="true" />
              </span>
              <h3 class="mt-4 text-lg font-semibold text-slate-100">{capability.title}</h3>
              <p class="mt-2 text-sm leading-relaxed text-slate-300">{capability.description}</p>
            </div>
          </article>
        {/each}
      </div>
    ...
    <Panel padding="lg" data-reveal class="reveal border-slate-700/70 bg-gradient-to-br from-slate-900/70 via-slate-900/65 to-indigo-950/20 backdrop-blur-sm space-y-6">
      ...
      <div class="grid grid-cols-1 xl:grid-cols-5 gap-4 items-stretch">
        <div class="xl:col-span-3 h-full rounded-2xl border border-sky-500/25 bg-gradient-to-br from-slate-950/80 to-slate-900/55 p-4 shadow-[0_10px_30px_rgba(56,189,248,0.12)] relative overflow-hidden flex flex-col justify-between">
          ...
          <div class="mt-3 flex-1 min-h-[260px] sm:min-h-[320px] rounded-xl border border-slate-800/70 bg-slate-950/70 p-3">
            <canvas bind:this={activityCanvas} class="h-full w-full"></canvas>
          </div>

          <p class="mt-3 text-[11px] text-slate-400">
            Real-time transaction flow based on simulated production patterns
          </p>

          <div class="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            <div class="rounded-lg border border-slate-700/70 bg-slate-900/45 px-3 py-2">
              <p class="text-[10px] uppercase tracking-[0.14em] text-slate-500">Avg tx/min</p>
              <p class="mt-1 text-sm font-semibold text-slate-100">{avgTransactionsPerMin}</p>
            </div>
            <div class="rounded-lg border border-slate-700/70 bg-slate-900/45 px-3 py-2">
              <p class="text-[10px] uppercase tracking-[0.14em] text-slate-500">Peak spike</p>
              <p class="mt-1 text-sm font-semibold text-slate-100">{peakTransactionsPerMin}</p>
            </div>
            <div class="rounded-lg border border-slate-700/70 bg-slate-900/45 px-3 py-2">
              <p class="text-[10px] uppercase tracking-[0.14em] text-slate-500">Volatility</p>
              <p class="mt-1 text-sm font-semibold text-slate-100">{volatilityIndicator}%</p>
            </div>
          </div>
        </div>

        <div class="xl:col-span-2 h-full grid grid-cols-1 gap-3 auto-rows-fr">
          {#each overviewMetrics as metric, index}
            <div class={`h-full rounded-xl border px-4 py-3 transition duration-300 hover:scale-[1.02]
              ${
                index === 0
